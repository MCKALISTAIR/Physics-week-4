...
void Collide(glm::vec3 c, glm::vec3 w, Particle &particle)
{
	//for (int i = 0; i < 3; i++)
	//{
	if (particle.getPos()[1] < c[1])
	{
		//set the particle in line with w
		particle.setPos(1, c[1]);
		//makes ball go opposite direction
		particle.setVel(1, particle.getVel()[1] *= -0.5f);
	}
	if (particle.getPos()[1] > c[1] + w[1])
	{
		//same as above
		particle.setPos(1, c[1] + w[1]);
		particle.setVel(1, particle.getVel()[1] *= -0.5f);
	}
	//}
}
...
//dimensions of cube
glm::vec3 c = glm::vec3(-2.5, 0.0f, 2.5f);
...
while (!glfwWindowShouldClose(app.getWindow()))
{
	...
	*/
	...
	//Collide(c, w, particle1);
	//Collide(c, w, particle2);
	//particle1.setPos();
	/*
	**	RENDER
	*/
